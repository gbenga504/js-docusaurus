    ...
    private validateComment(document: TextDocument, match: RegExpExecArray, functionLineNumber: number): Diagnostic | null {
        ...
        if (isLineACommentRegex.test(commentAtLine) === false) {
            ...
            return new Diagnostic(
                new Range(functionLineNumber, 0, functionLineNumber, document.lineAt(currentLine).text.length),
                `exported function ${functionName} should have comment or be unexported`,
                DiagnosticSeverity.Warning
            )
        }
        else {
            while (currentLine - 1 >= 0 && isLineACommentRegex.test(document.lineAt(currentLine - 1).text) === true) {
                currentLine -= 1;
            ...
            if (commentNameRegex.test(commentAtLine) === false) {
                ...
                return new Diagnostic(
                    new Range(currentLine, 0, functionLineNumber - 1, document.lineAt(functionLineNumber - 1).text.length),
